...
# 每批次tensor对应的标签
Y = tf.placeholder(tf.float32, [None, output_step, output_size])
# 输入层、输出层权重、偏置
weights = {
...
def lstm(batch):  # 参数：输入网络批次数目
    ...
    input_rnn = tf.reshape(input_rnn, [-1, input_step, rnn_unit])
        ...
        for i in range(training_iters):
            ...
            while(end < len(train_x)):
                _, loss_ = sess.run([train_op, loss], feed_dict={
                                    X: train_x[start:end], Y: train_y[start:end], keep_prob: 0.99})
                start += batch_size
                end = start+batch_size
                # 每10步保存一次参数
                if step % 10 == 0:
