...
class Futures{
    ...
    static async index(req,res,next){
        ...
        res.locals.page_title="View-"+goods.name;
        //找不到商品返回主页
        if(goods.length == 0)
            return res.redirect("/");
        else
    ...
    static async data(req,res,next){
        ...
        if(data.length > 0)
            return res.json({status:1,msg:"Get Data Succfully",data:data});
        else
            return res.json({status:0,msg:"No data"});
    }
    ...
    static async predict(req,res,next){
        ...
        exec(cmd,function(error,stdout,stderr){
            if(error) {
                ...
                res.json({status:1,msg:"Finish Predicting",data:stdout});
            }
        });
    }
}
