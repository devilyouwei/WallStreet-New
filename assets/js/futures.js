$(function(){
    ...
    async function get_predict_data(g_id){
        //深度学习预测
        console.log(tf)
        let model = await tf.loadLayersModel(`/tf_models/${g_id}/model.json`)
        ...
        $("#pre_data").text("Predicted: "+a);
        let now_data = $("#now_data").text();
        let percent = (a-now_data)/now_data
        $("#percent_size").text("Percent: "+percent)
    }
    ...
    function get_predict_data_old(g_id){
        ...
        $.get(url,function(res){
            if(res.status){
                $("#pre_data").text("Predict: "+res.data);
                let now_data = $("#now_data").text();
    ...
    function init_data_paint(g_id,limit,chart){
        ...
        $.get("/futures/data/"+g_id+"/"+limit,function(res){
            ...
            if(res.status == 0){
                ...
                switch(chart){
                    case "area-line":
                        paint_area_line(date,latest_price,chart1,"Closing Price");
                        paint_area_line(date,open_price,chart2,"Opening Price");
                        break;
                    case "area-bar":
                        paint_area_bar(date,latest_price,chart1,"Closing Price");
                        paint_area_bar(date,open_price,chart2,"Opening Price");
                        break;
                    default:break;
                }
                //最低最高折线
                paint_line(date,[{title:"Lowest",data:min_price},{title:"Highest",data:max_price},{title:"Volumes",data:volume}],chart3,"Min-Max Price");
            }
        });
    }
